...
import { loginUser, registerUser } from '../../../redux/slices/features/auth/authSlice'
...
const AuthForm: React.FC<AuthFormProps> = ({mode, role}) => {
  ...
  return (
    <div className='grid md:grid-cols-2 gap-0'>
       ...
       <div>
         {/* Google button */}
         <button className='bg-white rounded-full py-2 px-6 flex items-center justify-center gap-3 font-medium mb-6 hover:shadow-lg transition'>
         ...
         <form className='space-y-4' onSubmit={handleSubmit}>
            ...
            {mode === 'signup' && (
                <div>
                    ...
                    <input 
                       type='text'
                       className={`w-full bg-white border ${error.name ? 'border-red-500' : 'border-white'} rounded-xl px-4 py-3 text-black focus:outline-none transition focus:border-${accentColor} -500`} 
            ...
            <div>
                ...
                <input 
                    type="email" 
                    className={`w-full bg-white border ${error.email ? 'border-red-500' : 'border-white'} rounded-xl px-4 py-3 text-black focus:outline-none transition focus:border-${accentColor}-500`}
            ...
            <div>
                ...
                <input 
                    type="password" 
                    className={`w-full bg-white border ${error.password ? 'border-red-500' : 'border-white'} rounded-xl px-4 py-3 text-black focus:outline-none transition focus:border-${accentColor}-500`}
                    placeholder="••••••••"
            ...
            {mode === 'signup' && (
                <div>
                    ...
                    <input 
                        type="password" 
                        className={`w-full bg-white border ${error.confirmPassword ? 'border-red-500' : 'border-white'} rounded-xl px-4 py-3 text-black focus:outline-none transition focus:border-${accentColor}-500`}
                        placeholder="••••••••"
            ...
            <button 
                type='submit'
                disabled={loading}
                className={`w-full ${accentClass.split(' ').slice(2).join(' ')} text-white font-bold py-3 rounded-xl mt-4 transition duration-200 disabled:opacity-50 shadow-lg shadow-blue-900/20`}
                >
